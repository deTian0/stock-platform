...
| — | Workbench UI 日用抛光（渐进披露 + 推荐卡片） | 小 | `v3.9.1` | done |
| — | Workbench UI 结构化扫读（步骤/键值/状态） | 小 | `v3.9.2` | done |
...
**状态**：done（Phase E 收口 `v3.8.0`；基线曾为 `v3.0.0`）
...
- [x] 向导路径 replay E2E 或 `test_app` 断言；无 live 默认文案
- [x] CHANGELOG `3.7.0`；ADR（建议 0045→实际 0046，0045 已用于 LLM）
